...
try:
    from ..config.gemini_review_config import ReviewConfig, find_config_file, create_default_config
...
Be thorough and skeptical. Look for discrepancies between the claims and the actual implementation."""
    ...
    def save_results(self, results: str, output_path: str = "gemini-review.md", claims_file: Optional[str] = None):
        ...
        # Copy claims file to the same directory if provided
        if claims_file and Path(claims_file).exists():
            claims_dest = output_dir / "validation_claims.md"
            import shutil
            shutil.copy2(claims_file, claims_dest)
            print(f"📋 Copied validation claims to {claims_dest}")
        
        # Create validation timestamp log
        timestamp_log = output_dir / "validation_timestamp.log"
        with open(timestamp_log, 'w', encoding='utf-8') as f:
            f.write(f"Validation run: {time.strftime('%Y-%m-%d %H:%M:%S')}\n")
            f.write(f"Claims file: {claims_file if claims_file else 'None'}\n")
            f.write(f"Gemini review: {output_path}\n")
        
        with open(output_path, 'w', encoding='utf-8') as f:
            f.write(f"# Gemini Code Review\n")
            f.write(f"Generated on: {time.strftime('%Y-%m-%d %H:%M:%S')}\n\n")
            if claims_file:
                f.write(f"**Validation Claims**: {Path(claims_file).name}\n\n")
            f.write("---\n\n")
            f.write(results)
            
        print(f"✅ Results saved to {output_path}")
        print(f"📁 Validation directory: {output_dir}")
    ...
    def review(self, project_path: str = ".", 
              ...
              claims_file: Optional[str] = None) -> str:
        """Main review process."""
        print(f"\n🚀 Starting Gemini Code Review for: {project_path}\n")
        
        # Determine output directory based on claims file
        if claims_file and Path(claims_file).exists():
            claims_name = Path(claims_file).stem
            # Extract cycle number and description from claims filename
            # e.g., "cycle_25_phase1_integration" -> "cycle_25_phase1_integration"
            output_dir = Path("reports/validation_report") / claims_name
            output_path = output_dir / "gemini-review.md"
        else:
            # Create system-level review directory with timestamp
            timestamp = time.strftime("%Y%m%d_%H%M%S")
            output_dir = Path("reports/validation_report") / f"system_review_{timestamp}"
            output_path = output_dir / "gemini-review.md"
        ...
        try:
            ...
            # Step 5: Save results with claims file
            self.save_results(results, str(output_path), claims_file)
            
            # Step 6: Cleanup (unless asked to keep)
    ...
    try:
        ...
        if config:
            ...
            if args.claims:
                ...
                config.claims_file = args.claims  # Set claims_file to the same path
            if args.docs:
        ...
        if config:
            reviewer.review(
                ...
                claims_file=config.claims_file # Pass claims_file from config
            )
        else:
            reviewer.review(
                ...
                claims_file=args.claims # Pass claims_file path from command line
            )
